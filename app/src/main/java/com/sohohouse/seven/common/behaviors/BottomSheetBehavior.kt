...
import com.sohohouse.seven.common.extensions.readBooleanUsingCompat
import com.sohohouse.seven.common.extensions.writeBooleanUsingCompat
...
open class BottomSheetBehavior<V : View> @JvmOverloads constructor(
    context: Context,
    ...
    var halfExpandedRatio = 0.5f
        set(value) {
            require(0 < value && value < 1) { "ratio must be a float value between 0 and 1" }

            field = value
            if (viewRef != null) calculateHalfExpandedOffset()
        }

    var expandedOffset: Int = 0
        get() = if (isFitToContents) fitToContentsOffset else field
        set(value) {
    ...
    init {
        ...
        val ta = context.obtainStyledAttributes(attrs, R.styleable.BottomSheetBehavior_Layout)
        val value = ta.peekValue(R.styleable.BottomSheetBehavior_Layout_behavior_peekHeight)
        peekHeight = if (value?.data == PEEK_HEIGHT_AUTO) {
            value.data
        } else {
        ...
        isHideable = ta.getBoolean(R.styleable.BottomSheetBehavior_Layout_behavior_hideable, false)
        isFitToContents =
            ta.getBoolean(R.styleable.BottomSheetBehavior_Layout_behavior_fitToContents, true)
        skipCollapsed =
            ta.getBoolean(R.styleable.BottomSheetBehavior_Layout_behavior_skipCollapsed, false)
        ta.recycle()
    ...
    override fun onRestoreInstanceState(parent: CoordinatorLayout, child: V, state: Parcelable) {
        state as SavedState
        super.onRestoreInstanceState(parent, child, state.superState ?: Bundle.EMPTY)

        isDraggable = state.isDraggable
        peekHeight = state.peekHeight
        isFitToContents = state.isFitToContents
        isHideable = state.isHideable
        skipCollapsed = state.skipCollapsed

        // Set state last. Intermediate states are restored as collapsed state.
        _state = if (state.state == STATE_DRAGGING || state.state == STATE_SETTLING) {
            STATE_COLLAPSED
        } else {
            state.state
        }
    ...
    fun isHalfExpanded(): Boolean = state == STATE_HALF_EXPANDED

    fun isCollapsed(): Boolean = state == STATE_COLLAPSED
    ...
    fun toggle() {
        if (isCollapsed()) expandHalf() else collapse()
    }
    ...
    override fun onDependentViewChanged(
        ...
        halfExpandedRatio = 1f - (dependency.height.toFloat() / parent.height)
        peekHeight = 0
    ...
    override fun onLayoutChild(
        ...
        when (state) {
            STATE_EXPANDED -> ViewCompat.offsetTopAndBottom(child, expandedOffset)
    ...
    override fun onStartNestedScroll(
        ...
        if (isDraggable && viewRef?.get() == directTargetChild &&
            (axes and ViewCompat.SCROLL_AXIS_VERTICAL) != 0
        ) {
            // Scrolling view is a descendant of the sheet and scrolling vertically.
            // Let's follow along!
    ...
    override fun onNestedPreScroll(
        ...
        if (ViewCompat.TYPE_NON_TOUCH == type) {
            if ((dy > 0 && currentTop == expandedOffset) || (dy < 0 && currentTop == halfExpandedOffset)) return
        }
        ...
        if (dy > 0) { // Upward
            ...
            if (STATE_HALF_EXPANDED == state) return

            if (!target.canScrollVertically(-1)) {
                if (newTop <= halfExpandedOffset || skipCollapsed.not()) {
                    consumed[1] = dy
                    child.offsetTopAndBottom(-dy)
                    setStateInternal(STATE_DRAGGING)
                } else {
                    consumed[1] = currentTop - halfExpandedOffset
                    child.offsetTopAndBottom(-consumed[1])
                    setStateInternal(STATE_HALF_EXPANDED)
                }
            }
        }
    ...
    override fun onNestedPreFling(
        ...
        return isDraggable && target == nestedScrollingChildRef?.get()
    ...
    private val dragCallback: ViewDragHelper.Callback = object : ViewDragHelper.Callback() {
        ...
        override fun tryCaptureView(child: View, pointerId: Int): Boolean {
            ...
            if (STATE_HALF_EXPANDED == state && dy > 0) {
                // Disable downward movement if half expanded
                return false
            }
        ...
        override fun getViewVerticalDragRange(child: View): Int {
            val layoutParams = child.layoutParams ?: return 0
            if (layoutParams !is CoordinatorLayout.LayoutParams || layoutParams.behavior == null) return 0

            return if (isHideable) parentHeight else halfExpandedOffset
        }

        override fun clampViewPositionVertical(child: View, top: Int, dy: Int): Int {
            val maxOffset = if (isHideable) parentHeight else halfExpandedOffset
