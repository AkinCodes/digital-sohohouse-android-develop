...
class CoreRequestFactoryTest {
    private lateinit var requestFactory: CoreRequestFactory
    ...
    fun setUp() {
        ...
        requestFactory = CoreRequestFactory(
            api,
    ...
    @Test
    fun `server returns 201 (success no body)`() {
        // GIVEN server returns 201, with an empty success body
        val successObject = null
        val response = mock<Response<String>> {
            on(it.body()).thenReturn(successObject)
            on(it.isSuccessful).thenReturn(true)
        }
        ...
        // THEN the output of the call is the valid success object
        testObserver.assertValue { it is Either.Empty }
    }

    @Test
    fun `server returns any non-success status`() {
        ...
        val request = mock<CoreAPIRequest<Any>> {
            on(it.createCall(api)).thenReturn(call)
        ...
        observer.assertValue {
            it == error(AuthError.INVALID_OAUTH_TOKEN)
        }
    }
